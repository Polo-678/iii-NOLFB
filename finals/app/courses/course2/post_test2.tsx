...
export default function StudentScreen() {
  ...
  useEffect(() => {
    const fetchQuestionsAndAttempts = async () => {
      const querySnapshot = await getDocs(collection(db, "Post-test_2"));
      ...
      const scoresRef = collection(db, "Post-test_2_scores");
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          await setDoc(
            doc(db, "Post-test_2_scores", existingDoc.id),
            {
          ...
          await setDoc(doc(db, "Post-test_2_scores", studentEmail), {
            student: studentEmail,
            score: updatedScore,
            totalQuestions: questions.length,
            timestamp: new Date(),
